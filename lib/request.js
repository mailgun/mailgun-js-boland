...
debugLib.enable('mailgun-js');
var debug = debugLib('mailgun-js');
...
Request.prototype.handleResponse = function (res) {
  ...
  res.on('end', function () {
    ...
    if (!contentType) {
      debug('"content-type" not set in response headers. Available headers: %s', res.headers);
      debug('Request payload: %s', self.payload);
    }

    debug('response status code: %s content type: %s error: %s', res.statusCode, contentType, error);
    ...
    if (chunks && !error && (skipContentTypeCheck || (contentType && contentType.indexOf('application/json') >= 0))) {
      try {
