...
Request.prototype.handleResponse = function (res) {
  ...
  res.on('end', function () {
    ...
    if (!contentType) {
      console.log('Content-Type not set in response headers. Available headers:', res.headers);
      contentType = '';
    }
    ...
    if (chunks && !error && (skipContentTypeCheck || (contentType.indexOf('application/json') >= 0))) {
      try {
