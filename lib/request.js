...
Request.prototype.request = function (method, resource, data, fn) {
  ...
  if (this.retry > 1) {
    this.callback = function (err, body) {
      return fn(err, body);
    };

    retry(this.retry, function () {
...
Request.prototype.prepareData = function (data) {
  ...
  for (var key in data) {
    if (key !== 'attachment' && key !== 'inline') {
      params[key] = getDataValue(key, data[key]);
    }
  }
  ...
  for (var key in data) {
    ...
    if (key === 'attachment' || key === 'inline') {
      ...
      function appendKey(element) {
        self.form.append(key, getDataValue(key, element));
      }

      obj.forEach(appendKey);
    }
    else {
      this.form.append(key, getDataValue(key, obj));
    }
  }
